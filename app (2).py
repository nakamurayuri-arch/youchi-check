...
def a12_status(lat, lon, addr):
    ...
    if not pc:
        return {"err": "都道府県を特定できず（住所取得に失敗）"}
    code, pname = pc
    try:
        shps = download_a12(code)
    except Exception as e:
        return {"err": f"A12取得失敗（{code}）: {e}"}
    import shapefile
    for shp in shps:
        try:
        ...
        for sr in r.iterShapeRecords():
            ...
            if rings and _pip_ring(lon, lat, rings[0]) and not any(_pip_ring(lon, lat, h) for h in rings[1:]):
                ...
                return {"hit": True, "pref": pname, "file": shp.split("/")[-1], "attrs": attrs}
    return {"hit": False, "pref": pname}
...
if st.button("▶ チェックする", type="primary"):
    ...
    if a12.get("err"):
        ...
        st.write(f"- 青地/白地： **⚠ 農業地域/農用地区域に該当（青地の可能性）**")
